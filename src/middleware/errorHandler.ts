...
export const errorHandler = (err: Error, req: Request, res: Response, next: NextFunction) => {
  ...
  if (err.name === "ValidationError") return res.status(status.BAD_REQUEST).json({ success: false, message: err.message });

  if (err instanceof error.CustomError && err?.code === "EAUTH")
